%this is percent test, 
%40% no - guaranteed does not have
%60% yes - suspected (low confidence)
%80% yes - guaranteed (high confidence)
...
high_symp(X) :- symptom(X), unknown(X), asserta(has(X)), !.
...
no(D) :- 
    percent_no(D, P),
    P > 0.4, !.
unknown(D) :- 						%if there are no unknowns,
    percent_no(D, N),				%percent_yes + percent_no = 1
    percent_high(D, Y), ( 			%if no > 0.4, guaranteed no
    	N =< 0.2, Y < 0.8 ;			%if yes >= 0.8, guaranteed high
		0.2 < N, N =< 0.4, Y < 0.6	%if yes >= 0.6 and no <= 0.2, can be high
    ),!.							%if no <= 0.4, can be yes
ysus(D) :- disease(D), high(D), once((
    clause(sus(D), B),
	symptom_list(B, L),
	percent_hds(L, P),
    P >= 0.6
...
all_hd(X) :- findall(D, (disease(D), high(D)), X).
...
all_hs(X) :- findall(S, (symptom(S), high(S)), X).
...
ah(S) :- high_symp(S). % Asserts a 'high' symptom into the knowledge base.
an(S) :- no_symp(S). % Asserts a 'no' symptom into the knowledge base.
d :- all_d(X), write(X), nl. % Lists all known diseases.
hd :- all_hd(X), write(X), nl.
yd :- all_yd(X), write(X), nl. 
nd :- all_nd(X), write(X), nl. 
ud :- all_ud(X), write(X), nl. 
s :- all_s(X), write(X), nl. % Lists all known symptoms.
hs :- all_hs(X), write(X), nl. 
ys :- all_ys(X), write(X), nl. 
ns :- all_ns(X), write(X), nl. 
us :- all_us(X), write(X), nl. 
ds(D) :- all_ds(D, X), write(X), nl. % Lists all symptoms for a given disease.
hds(D) :- all_hds(D, X), write(X), nl.
yds(D) :- all_yds(D, X), write(X), nl. 
nds(D) :- all_nds(D, X), write(X), nl. 
uds(D) :- all_uds(D, X), write(X), nl. 
